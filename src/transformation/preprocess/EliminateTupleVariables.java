...
public class EliminateTupleVariables implements ExprTransformer {
	...
	/**
	 * column name to generate the new var
	 */
	...
	/**
	 * Name of the table used to replace the last variable
	 */
	...
	private void apply() {
		...
		for (int i = decls.size() - 1; i >= 0; i--) {
			...
			if (decl instanceof VarDecl) {
				...
				if (delete) {
					Replacement repl = new Replacement(this.sqlName, (VarDecl) decl, newDecls, columnName, mixed);
					replaced.add(repl);
					decls.remove(i);
				}
			}
		...
		// eliminate table declarations
		mp.emptyTables();
	}

	/**
	 * Replaces the i-th variable declaration, if its a tuple, by the jndividual
	 * components of its corresponding table
	 * 
	 * @param i:
	 *            position of the declaration analyzed in the list decls of the
	 *            model
	 * @param type:
	 *            type defining the declaration
	 * @param name:
	...
	private boolean replaceTupleVar(Type type, String name) {
		...
		boolean typeTuple = isTypeID || isTypeArray || isTypeSet;
		...
		if (isTypeID)
			replacement = checkType((TypeID) type);
		if (isTypeArray) {
	...
	private String newVarName(String originalName, String colName, boolean decl) {
		...
		// if this for a variable declaration we need to check that the name is
		// really new
	...
	public PreprocessResult getReplaced() {
